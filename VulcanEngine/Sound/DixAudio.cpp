...
class DixAudio::Impl {
   public:
    ...
    Impl() : sound{nullptr}, volume{1.f}, pan{0.f}, isSoundLoaded{false} {
        ma_result result = ma_engine_init(nullptr, &engine);
        if (result != MA_SUCCESS) {
            DixLogErr("Failer to initialize multiaudio engine, result = {}",
    ...
    bool isLoaded() const { return isSoundLoaded; }
    ...
    void updateListener(const glm::vec3& objPosition, const glm::vec3& position,
                        const glm::vec3& forwardDir, const glm::vec3& upDir) {
        if (!isLoaded()) {
            DixLogWarn(
                "Tried to update Listener position when no sound is loaded!");
            return;
        }
        ma_sound_set_position(sound, objPosition.x, objPosition.y,
                              objPosition.z);

        ma_engine_listener_set_position(&engine, 0, position.x, position.y,
                                        position.z);

        ma_engine_listener_set_direction(&engine, 0, forwardDir.x, forwardDir.y,
                                         forwardDir.z);
...
DixAudio::DixAudio(const std::string& filepath)
...
void DixAudio::play(bool loop) { m_Impl->play(loop); }

void DixAudio::stop() { m_Impl->stop(); }

void DixAudio::pause() { m_Impl->pause(); }

void DixAudio::resume() { m_Impl->resume(); }

void DixAudio::setVolume(float volume) { m_Impl->setVolume(volume); }

float DixAudio::getVolume() const { return m_Impl->getVolume(); }

void DixAudio::setPan(float pan) { m_Impl->setPan(pan); }

float DixAudio::getPan() const { return m_Impl->getPan(); }

void DixAudio::seekTo(float seconds) { m_Impl->seekTo(seconds); }

float DixAudio::getPosition() const { return m_Impl->getPosition(); }

float DixAudio::getDuration() const { return m_Impl->getDuration(); }

bool DixAudio::isPlaying() const { return m_Impl->isPlaying(); }

bool DixAudio::isPaused() const { return m_Impl->isPaused(); }

bool DixAudio::isLoaded() const { return m_Impl->isLoaded(); }

void DixAudio::setMinDistance(float dist) { m_Impl->setMinDistance(dist); }
void DixAudio::setMaxDistance(float dist) { m_Impl->setMaxDistance(dist); }
void DixAudio::updateListener(const glm::vec3& objPosition,
