...
std::unique_ptr<DixDescriptorSetLayout> DixDescriptorSetLayout::Builder::build()
    const {
...
DixDescriptorSetLayout::DixDescriptorSetLayout(
    ...
    auto result = engineDevice.device().createDescriptorSetLayout(
...
DixDescriptorPool::Builder& DixDescriptorPool::Builder::setMaxSets(
    uint32_t count) {
    maxSets = count;
    return *this;
}
...
std::unique_ptr<DixDescriptorPool> DixDescriptorPool::Builder::build() const {
    return std::make_unique<DixDescriptorPool>(engineDevice, maxSets, poolFlags,
                                               poolSizes);
}

DixDescriptorPool::DixDescriptorPool(
    EngineDevice& engineDevice, uint32_t maxSets,
    ...
    auto result =
        engineDevice.device().createDescriptorPool(descriptorPoolInfo);
    // if (result != vk::Result::eSuccess) {
...
void DixDescriptorPool::freeDescriptors(
...
DixDescriptorWriter::DixDescriptorWriter(DixDescriptorSetLayout& setLayout,
...
DixDescriptorWriter& DixDescriptorWriter::writeBuffer(
    ...
    // store a copy owned by the writer and point the write entry to that
    // storage
...
DixDescriptorWriter& DixDescriptorWriter::writeImageSampler(
    uint32_t binding, vk::ImageView imageView, vk::Sampler sampler,
