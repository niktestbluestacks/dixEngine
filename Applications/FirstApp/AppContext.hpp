...
class AppContext {
   public:
    ...
    vk::RenderPass getSwapChainRenderPass() {
        return m_dixRenderer.getSwapChainRenderPass();
    }
    vk::CommandBuffer beginFrame() { return m_dixRenderer.beginFrame(); }
    void endFrame() { m_dixRenderer.endFrame(); }
    void beginSwapChainRenderPass(vk::CommandBuffer cb) {
        m_dixRenderer.beginSwapChainRenderPass(cb);
    }
    void endSwapChainRenderPass(vk::CommandBuffer cb) {
        m_dixRenderer.endSwapChainRenderPass(cb);
    }
   ...
   private:
    ...
    // rendering resources
    std::unordered_map<std::string, std::unique_ptr<DixDescriptorPool>>
        m_systemPool;

    std::unordered_map<std::string,
                       std::vector<std::vector<std::unique_ptr<DixBuffer>>>>
        m_systemUboBuffers;
    std::unordered_map<std::string, std::unique_ptr<DixDescriptorSetLayout>>
        m_systemSetLayouts;
    std::unordered_map<std::string, std::vector<vk::DescriptorSet>>
        m_systemDescriptorSets;
