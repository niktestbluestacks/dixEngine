...
class DixDescriptorPool {
   public:
    ...
    DixDescriptorPool(EngineDevice& engineDevice, uint32_t maxSets,
                      vk::DescriptorPoolCreateFlags poolFlags,
...
class DixDescriptorWriter {
   public:
    DixDescriptorWriter(DixDescriptorSetLayout& setLayout,
                        DixDescriptorPool& pool);

    DixDescriptorWriter& writeBuffer(uint32_t binding,
                                     vk::DescriptorBufferInfo* bufferInfo);
    DixDescriptorWriter& writeImage(uint32_t binding,
