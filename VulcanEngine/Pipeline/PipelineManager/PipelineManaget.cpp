// dix
#include <Pipeline/PipelineManager/PipelineManadger.hpp>
...
PipelineManager::PipelineManager(EngineDevice& device, const PipelineConfigInfo& config)
    : device(device), vertShaderStageInfo(createShaderStage(VK_SHADER_STAGE_VERTEX_BIT, config.shaderStages.front().module)),
      fragShaderStageInfo(createShaderStage(VK_SHADER_STAGE_FRAGMENT_BIT, config.shaderStages.back().module)), vertexInputInfo(config.vertexInputInfo),
      inputAssemblyInfo(config.inputAssemblyInfo), viewportStateInfo{ VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO },
      rasterizerInfo(createRasterizationState()), multisampleInfo(createMultisampleState()),
      depthStencilInfo(createDepthStencilState()) {
