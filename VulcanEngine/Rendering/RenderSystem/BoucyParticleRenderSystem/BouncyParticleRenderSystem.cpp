...
BouncyParticleRenderSystem::BouncyParticleRenderSystem(
    ...
    : DixRenderSystem(
          ...
          DixRenderSystemConfig{
              ...
              // Point-list: each BouncyParticle is a screen-space point /
              // sprite.
              ...
              .compute =
                  ComputePipelineConfig{
                      ...
                      .bindings =
                          {
                              // binding 0: BouncyParticle SSBO (read/write by
                              // vertex + compute)
    ...
    m_simulationParamsBuffer = std::make_unique<DixBuffer>(
        engineDevice, sizeof(BouncyParticleSimulationParams), 1,
        vk::BufferUsageFlagBits::eUniformBuffer,
        vk::MemoryPropertyFlagBits::eHostVisible |
            vk::MemoryPropertyFlagBits::eHostCoherent);
    m_simParams.gravityDeltaTime.w = 0.016f;
    m_simulationParamsBuffer->map();
    m_simulationParamsBuffer->writeToBuffer(
        &m_simParams, sizeof(BouncyParticleSimulationParams));
...
void BouncyParticleRenderSystem::buildComputeDescriptors() {
    ...
    // BouncyParticle SSBO: skip the leading uint32_t BouncyParticle-count
    // header.
    vk::DescriptorBufferInfo particleInfo{};
    particleInfo.buffer = m_particleBuffer->getBuffer();
    particleInfo.offset = 0;
    particleInfo.range = 16 + sizeof(BouncyParticle) * MAX_PARTICLES;
